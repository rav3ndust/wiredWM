...
install_wired_pkgs () {	
	...
	sudo apt-get update
	sudo apt-get install -y i3 i3lock-fancy nitrogen pulseaudio-utils xcompmgr picom slock alacritty fonts-inter xterm calcurse arandr rofi xss-lock feh volumeicon-alsa polybar blueman dunst flameshot meteo-qt pasystray ffmpeg kitty stterm surf conky suckless-tools lxpolkit lxappearance vim nnn cmus xscreensaver amfora sway swaylock swaybg grimshot xdg-desktop-portal-wlr qt5ct yaru-waybar wf-recorder gsimplecal calcurse pavucontrol yaru-theme-gtk yaru-theme-icon
} 
apply_configs () {
