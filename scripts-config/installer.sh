...
install_wired_pkgs () {	
	...
	sudo apt-get update
	sudo apt-get install -y i3 i3lock-fancy nitrogen pamixer wget htop opendoas lsd nsxiv pulseaudio-utils xcompmgr picom waybar slock alacritty fonts-inter xterm arandr nemo rofi xss-lock feh pandoc volumeicon-alsa polybar blueman dunst flameshot meteo-qt pasystray ffmpeg kitty stterm surf conky-all suckless-tools lxpolkit lxappearance vim nnn cmus cava xscreensaver amfora sway swaylock swayidle swaybg grimshot xdg-desktop-portal-wlr qt5ct tty-clock wf-recorder sakura foot gsimplecal calcurse pavucontrol yaru-theme-gtk yaru-theme-icon glow pipx wl-clipboard wlr-randr jq imagemagick tmux shotman nwg-look fonts-jetbrains-mono fonts-firacode fonts-noto wdisplays ydotool papirus-icon-theme fonts-font-awesome fonts-material-design-icons-iconfont bibata-cursor-theme cmatrix lynx elinks w3m libnotify-bin
} 
apply_configs () {
